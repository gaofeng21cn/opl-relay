...
它既是 OPL Persona 数字分身的通信模块，也可以作为独立插件和邮件工具使用。
...
| 层次 | 保存什么 | 由谁管理 |
| --- | --- | --- |
| Git 仓库 | 源码、测试、插件文件、Skills 和 Package 描述 | Git 与维护者 |
...
源码来自哪里，不是更新源；GitHub 上的源码提交也不能直接当作不可变的 OPL 发布包。
...
- `codex-mail` 继续作为兼容命令保留；新集成应使用 `opl-relay`。
